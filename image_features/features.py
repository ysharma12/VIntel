"""This script defines class and methods for feature computation and
   filtering out images not suitable for training"""
...
class ImageFeatures():
    """Computes discrete fourier transform of an image to capture image frequencies.
       The dft image is used to compute the image texture features using
...
#        discrete_fourier_transform/discrete_fourier_transform.html
    ...
    def get_glcm_features(self, input_image):
        ...
        glcm = greycomatrix(image, [5], list(np.arange(0, 360, 30)),
    ...
    def update_results(self, result, glcm_props, quality_label, image_name, class_label):
        """Updates the glcm properties with data for current image."""
...
#                     print(height, width)
                    ...
                    self.glcm_props = self.update_results(result, self.glcm_props,
                                                          label, image_filename, cls)
